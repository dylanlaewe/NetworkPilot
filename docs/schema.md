...
Migration `0013` adds the explicit Gmail send state machine and protected send audit. `gmail_draft_operations` records `not-sent`, `sending`, `sent`, or `send-status-uncertain`, the confirmed send time, and Gmail response identity. State and audit transitions are transactional. The migration broadens manual-outreach provenance only enough to distinguish a Gmail-confirmed NetworkPilot send from the existing operator-confirmed external fallback; historical rows and immutable snapshots retain their original values.
...
Migration `0015` adds `candidate_refresh_events` and `discovered_companies`. Refresh events are immutable product/accounting records with reserve target, provider cap, search calls, enrichment credits, additions, and qualified additions. Discovered companies remain separate from the curated preferred registry and store normalized identity, optional domain, source, deterministic legitimacy state, and first/last-seen timestamps. They contain no provider payload or credential.
