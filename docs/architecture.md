...
Historical `targeting-v1` plans retain their original eight named weighted components, explanations, totals, and hard-gate results. New planning uses `targeting-v2`, which consumes the provider-neutral `recipient-relevance-v1` snapshot. That component scores supported function affinity and grants a bounded exact-target-role bonus when a precise analogue exists. Lack of an exact analogue no longer zeroes a functionally relevant recipient. Industry and geography are preferences rather than absolute filters. Weights must be finite, non-negative, and total 100; ranking ties resolve by stable candidate ID.
