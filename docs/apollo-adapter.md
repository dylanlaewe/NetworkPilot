...
The Headquarters-authorized `npm run apollo:operational-scale` operator is also local-only and one-shot. It searches 24 existing reviewed Tier 1/2 companies with one page of at most 25 records each, processes no more than 750 raw records, and shortlists no more than 40 candidates with a two-person company cap and an eight-person primary-function soft cap. It permits at most 40 exact-ID People Match requests and two usage snapshots, preauthorizes at most 40 lead credits, disables retries, and stops enrichment if a response contradicts the approved one-credit disabled-phone model. Personal-email, phone, waterfall, bulk, organization-enrichment, inbox, and delivery capabilities are absent. Its search and enrichment databases are under `data/`, remain Git-ignored, and must never be committed.
