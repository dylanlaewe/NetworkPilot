...
console.table(entries.map((entry)=>({"Operator ID":entry.operatorId,Recipient:entry.redactedRecipient,Company:entry.company,Title:entry.title,Subject:entry.subject,"Gmail draft created":"yes","Send recorded":entry.manualSendConfirmed?"yes":"no","Send provenance":entry.confirmationSource==="networkpilot-gmail-send"?"Gmail-confirmed NetworkPilot send":entry.confirmationSource==="operator"?"Operator-confirmed external send":"—","Effective sent time":entry.effectiveSentAt??"—",Outcome:entry.outcome??"—",Suppressed:entry.suppressed?"yes":"no","Response state":entry.responseState??"—"})));
