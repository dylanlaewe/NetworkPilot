...
import { CappedApolloEnrichmentTransport, CONTROLLED_ENRICHMENT_MAX_CREDIT_EXPOSURE, runControlledApolloEnrichment } from "../src/application/providers/run-controlled-apollo-enrichment";
...
const attemptKey = (id: string) => `apolloLiveEnrichmentAttempted:${createHash("sha256").update(id).digest("hex")}`;
const expectedFailClosed = new Set(["provider-match-confidence-unavailable", "provider-match-confidence-malformed", "provider-identity-mismatch"]);
...
async function main(): Promise<void> {
  ...
  const transport = new CappedApolloEnrichmentTransport(new FetchApolloTransport());
  try {
    search.migrate(); repository.migrate();
    if (repository.getSetting("apolloLiveEnrichmentCompleted") === "true") throw new Error("controlled-apollo-enrichment-already-completed");
    ...
    const environment = { ...process.env, NETWORKPILOT_APOLLO_MAX_ENRICHMENTS_PER_BATCH: "3", NETWORKPILOT_APOLLO_MAX_ENRICHMENTS_PER_DAY: "3", NETWORKPILOT_APOLLO_MAX_RETRIES: "1" };
    ...
    repository.setSetting("apolloLiveEnrichmentCompleted", "true", new Date());
    console.log(JSON.stringify({ operation: "NetworkPilot controlled live enrichment", logicalEnrichmentsThisInvocation: candidates.length + failures.length, httpAttemptsThisInvocation: transport.requestCount, endpoint: "https://api.apollo.io/api/v1/people/match", maximumCreditExposure: CONTROLLED_ENRICHMENT_MAX_CREDIT_EXPOSURE, failures, candidates: candidates.map((candidate) => ({ apolloId: candidate.source.providerRecordId, matchConfidence: candidate.source.providerMetadata?.matchConfidence ?? "unknown", fullNameMapped: Boolean(candidate.source.person.firstName && candidate.source.person.lastName), title: candidate.source.currentTitle, organization: candidate.source.currentOrganization.name, organizationDomain: candidate.source.currentOrganization.domain ?? "unknown", location: candidate.source.location, experience: candidate.experience, role: candidate.classification, persona: candidate.personaId, emailReturned: Boolean(candidate.source.email.address), emailDomain: candidate.source.email.address.split("@")[1] ?? "unknown", apolloEmailStatus: candidate.source.email.verificationStatus, networkPilotVerification: candidate.source.email.verificationStatus, companyMatch: candidate.strategyCompanyMatch, dataQuality: candidate.dataQualityScore, hardGateFailures: candidate.gateFailures, reviewState: candidate.reviewState, lifecycleState: candidate.state, targetingEligible: candidate.state === "eligible", planningEligible: false })) }, null, 2));
