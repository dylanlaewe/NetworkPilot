...
function stateAction(state: DraftState) {
  ...
  if (state === "gmail-created") return { label: "Send email", key: "S" };
      ...
      <section className={styles.nowPlane}>
        ...
        {allCaughtUp ? <EmptyState title="Your active queue is clear" body="Relationship memory remains available, and the next candidate refresh can begin when needed." action="Review candidates" onAction={() => onNavigate("candidates")} /> : (
          <div className={styles.taskList}>
            {tasks.map((task, index) => <button key={task.id} data-rank={index + 1} className={selectedTask === task.id ? styles.selectedTask : ""} onClick={() => setSelectedTask(task.id)}><i>{String(index + 1).padStart(2, "0")}</i><span><strong>{task.title}</strong><small>{task.detail}</small></span><b>{task.count}</b></button>)}
          </div>
        )}
...
function CandidatesView(props: CandidateProps) {
  ...
  return <div className={styles.candidatesView}>
    ...
    {props.candidates.length === 0 ? <EmptyState title="No candidates match this view" body="The current filters are still visible. Clear them to restore the fictional opportunity pool." action="Clear filters" onAction={props.onClear} /> : <div className={styles.candidateWork}><div className={styles.candidateList}><div className={styles.candidateColumns}><span>Person and current work</span><span>Why now</span><span>Context</span></div>{props.candidates.map((candidate) => <button key={candidate.id} className={`${styles.candidateRow} ${props.selectedId === candidate.id ? styles.selectedCandidate : ""}`} onClick={() => props.onSelect(candidate.id)}><span className={styles.candidateProfile}><span className={styles.candidateIdentity}><strong>{candidate.person}</strong><small>{candidate.track}</small></span><span className={styles.candidateCurrent}><strong>{candidate.role}</strong><small>{candidate.company}</small></span></span><span className={styles.candidateCue}>{candidateCue(candidate)}</span><span className={styles.candidateMeta}><span className={styles.candidateLane}>{candidate.lane}</span><em data-suppressed={candidate.availability === "Suppressed" ? "true" : undefined}><i aria-hidden="true">{candidate.availability === "Suppressed" ? "×" : "·"}</i>{candidate.availability}</em></span></button>)}</div>{props.selected ? <aside className={`${styles.candidateInspector} ${props.mobileDetail ? styles.candidateDetailOpen : ""}`}><button className={styles.candidateClose} onClick={props.onCloseDetail}>Close</button><small>Selected candidate</small><h2>{props.selected.person}</h2><p className={styles.candidateRole}>{props.selected.role}</p><p className={styles.candidateCompany}>{props.selected.company}</p><dl><div><dt>Track</dt><dd>{props.selected.track}</dd></div><div><dt>Location</dt><dd>{props.selected.location}</dd></div><div><dt>Availability</dt><dd>{props.selected.availability}</dd></div></dl><section><h3>Why this person</h3><p>{props.selected.relevance}</p></section><section><h3>Company evidence</h3><p>{props.selected.companyEvidence}</p></section><section><h3>History</h3><p>{props.selected.history}</p></section><section className={props.selected.availability === "Suppressed" ? styles.suppressedBlock : styles.eligibleBlock}><h3>Planning eligibility</h3><strong>{props.selected.availability === "Suppressed" ? "Permanently excluded" : "Eligible for a future plan"}</strong><p>{props.selected.availability === "Suppressed" ? "Ordinary review cannot override this suppression." : "Company cooldown and one-per-company rules still apply at selection time."}</p></section></aside> : null}</div>}
  </div>;
}
...
function CommandPalette({ view, selected, onClose, onNavigate, onApprove, onSkip, onReplace, onAdd, onFilter, onSearch }: { view: View; selected: DraftFixture | null; onClose: () => void; onNavigate: (view: View) => void; onApprove: () => void; onSkip: () => void; onReplace: () => void; onAdd: () => void; onFilter: () => void; onSearch: () => void }) {
  ...
  return <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><section className={styles.commandPalette} role="dialog" aria-modal="true" aria-label="Command palette"><header><strong>Network command</strong><span>{view}{selected ? ` / ${selected.person}` : " / workspace"}</span></header><label><span className={styles.srOnly}>Search commands</span><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Where do you want to work?" /><kbd>Esc</kbd></label><div>{["Navigation", "Context"].map((group) => <section key={group}><h2>{group === "Context" ? "Selected work" : group}</h2>{items.filter((item) => item.group === group).map((item) => <button key={item.label} className={item.current ? styles.currentCommand : ""} disabled={Boolean(item.disabled)} onClick={() => { item.action?.(); onClose(); }}><span><strong>{item.label}</strong>{item.current ? <small>Current workspace</small> : item.disabled ? <small>{item.disabled}</small> : null}</span>{item.hint ? <kbd>{item.hint}</kbd> : null}</button>)}</section>)}</div><footer><span>↑↓ choose</span><span>↵ run</span><span>Esc close</span></footer></section></div>;
}

function SendConfirmation({ selected, onCancel, onConfirm }: { selected: DraftFixture | null; onCancel: () => void; onConfirm: () => void }) {
  return <div className={styles.modalBackdrop}><section className={styles.sendDialog} role="dialog" aria-modal="true" aria-labelledby="send-title"><small>Final review · simulation only</small><h2 id="send-title">Send email</h2><div className={styles.sendRecipient}><small>To</small><span>{selected?.person}</span><em>{selected?.role}</em><strong>{selected?.company}</strong></div><div className={styles.sendSubject}><small>Subject</small><p>{selected?.subject}</p></div><p className={styles.sendConsequence}>This is the external step: in production, the approved message would leave NetworkPilot and could not be recalled here.</p><dl><div><dt>Attachment</dt><dd>{selected?.resume}</dd></div><div><dt>Approved snapshot</dt><dd>v{selected?.version}</dd></div><div><dt>Prototype boundary</dt><dd>No provider call</dd></div></dl><div><button onClick={onCancel}>Cancel</button><button onClick={onConfirm}>Send email</button></div></section></div>;
}
