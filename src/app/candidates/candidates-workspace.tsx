...
export function CandidatesWorkspace({candidates,initialSelectedId,initialFilterOpen=false,refreshControl,refreshResult}:{candidates:ReserveCandidate[];initialSelectedId?:string;initialFilterOpen?:boolean;refreshControl:ReactNode;refreshResult:string|null}){
  ...
  return <div className={styles.candidates} data-density={density}>
    <header className={styles.header}>
      ...
      {filterOpen?<div className={styles.sheet} role="dialog" aria-label="Filter candidates"><header><strong>Filter candidates</strong><button type="button" onClick={()=>setFilterOpen(false)}>Done</button></header><Filter label="Track" value={filters.track} onChange={(track)=>setFilters((current)=>({...current,track}))} options={[["all","All tracks"],["professional","Professional"],["recruiter","Recruiter"]]}/><Filter label="Role family" value={filters.role} onChange={(role)=>setFilters((current)=>({...current,role}))} options={[["all","All roles"],...roles.map((role)=>[role,roleLabel(role)] as [string,string])]}/><Filter label="Industry" value={filters.industry} onChange={(industry)=>setFilters((current)=>({...current,industry}))} options={[["all","All industries"],...industries.map((value)=>[value,value] as [string,string])]}/><Filter label="Geography" value={filters.geography} onChange={(geography)=>setFilters((current)=>({...current,geography}))} options={[["all","All locations"],...geographies.map((value)=>[value,value] as [string,string])]}/><Filter label="Availability" value={filters.status} onChange={(status)=>setFilters((current)=>({...current,status}))} options={[["qualified-available","Available"],["all","All states"],["in-draft-queue","In Drafts"],["cooldown","Available later"],["suppressed","Do not contact"],["already-contacted","Contacted"],["enriched-unqualified","Not eligible"],["search-only","Needs verification"]]}/><button type="button" className={styles.clear} onClick={clear}>Clear all filters</button></div>:null}
    </header>
