...
export interface ApolloProviderRecordRejection {recordIndex:number;reason:string;}
export interface ApolloSearchResult {rawEvidence?:{candidateKeys:string[];employers:Array<{key:string;preferred:boolean}>;unknownEmployerRecords:number};records:CandidateSourceRecord[];recordRejections?:ApolloProviderRecordRejection[];totalAvailable:number|null;requestVersion:string;}
