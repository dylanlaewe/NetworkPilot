...
function diversifiedShortlist(candidates:readonly ImportedCandidateSnapshot[],maximum:number):ImportedCandidateSnapshot[]{
  const ranked=[...candidates].sort((a,b)=>candidateScore(b)-candidateScore(a)||a.id.localeCompare(b.id)),selected:ImportedCandidateSnapshot[]=[],companies=new Map<string,number>(),people=new Set<string>();
  const add=(track:"professional"|"recruiter",limit:number)=>{for(const candidate of ranked){if(selected.filter((item)=>item.outreachTrack===track).length>=limit)break;const company=candidate.strategyCompanyMatch?.companyId,id=candidate.source.providerRecordId;if(candidate.outreachTrack!==track||!company||(companies.get(company)??0)>=2||people.has(id))continue;selected.push(candidate);companies.set(company,(companies.get(company)??0)+1);people.add(id);}};
  add("recruiter",Math.min(5,maximum));add("professional",Math.max(0,maximum-selected.length));
  for(const candidate of ranked){if(selected.length>=maximum)break;const company=candidate.strategyCompanyMatch?.companyId,id=candidate.source.providerRecordId;if(!company||(companies.get(company)??0)>=2||people.has(id))continue;selected.push(candidate);companies.set(company,(companies.get(company)??0)+1);people.add(id);}
  return selected;
}
...
export class ApolloDailyReplenisher implements DailyRefreshReplenisher{
 ...
 async replenish(maximum:number):Promise<{candidates:RefreshCandidate[];attempts:number;searchCalls:number;searchRawPeople:number;searchValidNormalized:number;searchUniquePeople:number;creditBefore:number|null;creditAfter:number|null}>{
  ...
  if(!shortlist.length)return{candidates:[],attempts:0,searchCalls,searchRawPeople,searchValidNormalized,searchUniquePeople:searchedPeople.size,creditBefore:null,creditAfter:null};
  ...
  const after=await adapter.creditUsage();return{candidates:imported.map((candidate)=>({id:candidate.id,company:candidate.strategyCompanyMatch?.companyId??candidate.source.currentOrganization.name,track:candidate.outreachTrack??"professional",score:candidate.outreachTrack==="recruiter"?candidate.recruiterClassification?.score??0:candidate.dataQualityScore,available:candidate.state==="eligible"&&candidate.source.email.verificationStatus==="verified"&&candidate.gateFailures.length===0})),attempts:imported.length,searchCalls,searchRawPeople,searchValidNormalized,searchUniquePeople:searchedPeople.size,creditBefore:before.leadCreditsConsumed,creditAfter:after.leadCreditsConsumed};
 }
}
