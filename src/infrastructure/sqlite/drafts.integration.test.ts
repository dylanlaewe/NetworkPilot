...
describe("draft persistence and fictional targeting profiles", () => {
  ...
  it("generates idempotent rotated drafts with explicit version snapshots", () => {
    ...
    expect(second[0]).toMatchObject({ templateVersion: "10.0.0", templateCatalogVersion: "catalog-v10-dylan-outreach-method-v5", scoreVersion: "targeting-v2", score: first[0].score, createdAt: "2026-09-07T16:00:00.000Z" });
  ...
  it("copies the selected normalized plan exactly and cannot draft a rejected decision",()=>{const {repository,run}=setup();const plan=repository.findCampaignPlan(run.id)!;const drafts=generateDraftsForRun(repository,run.id,()=>new Date("2026-09-07T16:00:00Z"));for(const draft of drafts){const planned=plan.selected.find((item)=>item.prospectId===draft.prospectId)!;expect(draft.score).toBe(planned.totalScore);expect(draft.scoreComponents).toEqual(planned.components);expect(draft.explanationCodes).toEqual(planned.explanationCodes);expect(draft.recipientSnapshot).toMatchObject({normalizedTitle:planned.normalizedTitle,classificationVersion:planned.classificationVersion,classificationExplanationCodes:planned.classificationExplanationCodes,companyName:planned.fictionalEmployer.name,strategyCompanyName:planned.strategyCompanyMatch?.canonicalName,companyMatchMethod:planned.matchMethod});}const rejected=plan.decisions.find((item)=>!item.selected)!;repository.native.prepare("UPDATE campaign_plan_decisions SET selected=1 WHERE plan_id=? AND prospect_id=?").run(run.id,rejected.prospectId);expect(()=>generateDraftsForRun(repository,run.id,()=>new Date())).toThrow("not eligible in the persisted plan");expect(repository.listDrafts()).toHaveLength(drafts.length);repository.close();});
