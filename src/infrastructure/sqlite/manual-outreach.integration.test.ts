...
describe("operator-confirmed manual outreach production path", () => {
  ...
  it("hydrates recruiter-track Command Center operations without weakening unresolved identity handling",()=>{
    const {repository,databasePath}=setup(),candidate=repository.findImportedCandidate("fictional-flat","flat-001")!;
    const recruiterSnapshot:ApprovedEmailDraftSnapshot={snapshotId:"command-center:fixture-recruiter",recipientProfessionalEmail:"flat.one@example.test",recipientDisplayName:"Fictional F.",subject:"Recruiter subject",body:"Fictional body",planningSnapshotId:"daily-command-center:external-recruiter",templateCatalogVersion:"recruiter-v1",evidenceIds:[],approvedAt:"2026-09-09T12:00:00.000Z",outreachTrack:"recruiter",candidateId:"external-recruiter",companyId:candidate.strategyCompanyMatch!.companyId,companyDisplayName:"Imaginary Provider Lab",professionalTitle:"Data Engineer"};
    const recruiterOperation=approveForGmailDraft(repository,recruiterSnapshot,"fixture-adapter-v1","recruiter");markGmailDraftCreated(repository,recruiterOperation.operationId);
    const unresolvedSnapshot:ApprovedEmailDraftSnapshot={snapshotId:"command-center:unresolved",recipientProfessionalEmail:"missing@example.test",recipientDisplayName:"Missing M.",subject:"Unresolved subject",body:"Missing body",planningSnapshotId:"daily-command-center:missing-candidate",templateCatalogVersion:"recruiter-v1",evidenceIds:[],approvedAt:"2026-09-09T12:00:00.000Z"};
    ...
    const recruiterEntry=listManualDraftOperatorEntries(databasePath).find((entry)=>entry.snapshotId===recruiterSnapshot.snapshotId)!;
    expect(recruiterEntry).toMatchObject({outreachTrack:"recruiter",company:"Imaginary Provider Lab",title:"Data Engineer"});
    const confirmed=confirmManualSendByOperatorId({entries:[recruiterEntry],operatorId:recruiterEntry.operatorId,effectiveSentAt:SENT_AT,now:()=>CONFIRMED_AT,repository});
    expect(confirmed.outreachTrack).toBe("recruiter");
    expect(recordOperatorReportedHardBounce({snapshotId:recruiterSnapshot.snapshotId,effectiveSentAt:SENT_AT,now:()=>new Date("2026-09-09T15:00:00.000Z"),repository}).outcome).toBe("hard-bounce");
    expect(repository.native.prepare("SELECT DISTINCT outreach_track FROM manual_outreach_audit").all()).toEqual([{outreach_track:"recruiter"}]);
