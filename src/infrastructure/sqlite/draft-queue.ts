...
export function loadQueueReviews(repository:SqliteSimulationRepository,at=new Date()):CommandCenterDraftReview[]{
  ...
  for(const generation of generations){for(const [ordinal,id] of generation.candidateIds.entries()){const review=generation.draftReviews?.find(d=>d.candidateId===id)??(byId.has(id)?renderQueueReview(byId.get(id)!,ordinal,new Date(generation.createdAt)):null);if(review)base.set(id,{review,date:generation.campaignDate});}}
  ...
  const operations=(db.prepare("SELECT draft_snapshot_id FROM gmail_draft_operations ORDER BY updated_at_utc,operation_id").all() as {draft_snapshot_id:string}[]).map(row=>repository.findGmailDraftOperation(row.draft_snapshot_id)!);
  for(const operation of operations){const snapshot=operation.snapshot,id=snapshot.candidateId;if(!id||operation.sendState==="sent")continue;const prior=base.get(id),candidate=byId.get(id),review=prior?.review??(candidate?renderQueueReview(candidate,base.size,at):null);if(!review)continue;base.set(id,{date:prior?.date??snapshot.approvedAt.slice(0,10),review:{...review,operation,snapshotId:snapshot.snapshotId,subject:snapshot.subject,body:snapshot.body,wordCount:snapshot.body.trim().split(/\s+/).length,catalogVersion:snapshot.templateCatalogVersion,recipient:snapshot.recipientDisplayName,recipientEmail:snapshot.recipientProfessionalEmail,company:snapshot.companyDisplayName??review.company,title:snapshot.professionalTitle??review.title}});}
