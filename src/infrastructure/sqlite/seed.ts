...
export function seedTargetCompanyRegistry(repository:SqliteSimulationRepository):void{
...
export function seedFictionalData(repository: SqliteSimulationRepository): void {
  repository.transaction(() => {
    ...
    seedTargetCompanyRegistry(repository);
    for (let i = 1; i <= BASE_FICTIONAL_PROSPECT_COUNT; i += 1) {
      ...
      companyProfile.run(`fictional-company-${suffix}`,industryId,scenarioTier,55+(i*5)%41,60+(i*7)%41,50+(i*11)%51,"fictional-company-profile-v1",registryCompany?.id??null,"simulation-alias","Explicit fictional scenario alias; no real-company employment claim.",1);
