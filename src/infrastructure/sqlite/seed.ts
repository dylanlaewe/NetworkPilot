...
export function seedFictionalData(repository: SqliteSimulationRepository): void {
  repository.transaction(() => {
    ...
    for (let i = 1; i <= FICTIONAL_PROSPECT_COUNT; i += 1) {
      ...
      companyProfile.run(`fictional-company-${suffix}`,industryId,scenarioTier,55+(i*5)%41,60+(i*7)%41,50+(i*11)%51,"fictional-company-profile-v1");
