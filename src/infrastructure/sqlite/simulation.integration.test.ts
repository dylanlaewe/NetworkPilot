...
describe("SQLite fictional simulation", () => {
  ...
  it("keeps the persisted targeting plan idempotent and versioned",()=>{
    ...
    expect(second.existing).toBe(true);expect(repo.findCampaignPlan(first.id)).toEqual(snapshot);expect(snapshot).toMatchObject({planVersion:"campaign-plan-v1",targetingVersion:"targeting-v2",status:"planned"});repo.close();
  });
