...
export type ReserveStage = "in-draft-queue" | "search-only" | "enriched-unqualified" | "qualified-available" | "cooldown" | "suppressed" | "already-contacted";
...
export interface DailyCommandCenterState {
  ...
  safety:{suppressed:number;cooldownCompanies:number;gmailState:string;apolloEnabled:boolean;apolloReady:boolean;apolloExposure:number;apolloObserved:number|null};
}
export interface TrackAnalytics {attempts:number;replyEligible:number;hardBounces:number;replies:number;meetings:number;declines:number;optOuts:number;}
...
export function buildDailyCommandCenter(input:{drafts:ManualDraftOperatorEntry[];reserve:ReserveCandidate[];gmailState:string;apolloEnabled:boolean;apolloReady?:boolean;apolloExposure:number;apolloObserved:number|null;cooldownCompanies:number}):DailyCommandCenterState{
  ...
  const track=(value:ManualDraftOperatorEntry)=>value.outreachTrack??"professional",trackCounts=(name:"professional"|"recruiter")=>{const rows=input.drafts.filter((d)=>track(d)===name),attempts=rows.filter((d)=>d.manualSendConfirmed).length,hardBounces=rows.filter((d)=>d.outcome==="hard-bounce").length;return{attempts,replyEligible:rows.filter((d)=>d.outcome==="awaiting-response").length,hardBounces,replies:rows.filter((d)=>d.outcome==="replied").length,meetings:rows.filter((d)=>d.outcome==="meeting-scheduled").length,declines:rows.filter((d)=>d.outcome==="declined").length,optOuts:rows.filter((d)=>d.outcome==="opt-out").length};};
  return{outreach:{attempted,replyEligible:attempted-hardBounces,outcomes:counts,bounceRate:attempted?hardBounces/attempted:0},trackAnalytics:{professional:trackCounts("professional"),recruiter:trackCounts("recruiter")},pipeline:{sourced:candidates.length,preEnrichment:candidates.filter((c)=>c.stage==="search-only").length,enriched:candidates.filter((c)=>c.stage!=="search-only").length,qualified:candidates.filter((c)=>["in-draft-queue","qualified-available","cooldown","already-contacted"].includes(c.stage)).length,available:candidates.filter((c)=>c.stage==="qualified-available").length,gmailDraftCreated:input.drafts.length,manuallySent:attempted},drafts:input.drafts,reserve:candidates,recommendation,safety:{suppressed:candidates.filter((c)=>c.stage==="suppressed").length,cooldownCompanies:input.cooldownCompanies,gmailState:input.gmailState,apolloEnabled:input.apolloEnabled,apolloReady:input.apolloReady??input.apolloEnabled,apolloExposure:input.apolloExposure,apolloObserved:input.apolloObserved}};
}
