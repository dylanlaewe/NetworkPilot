import { CANDIDATE_SOURCE_TYPES, classifyCandidate, classifyRecipientFunction, CLASSIFICATION_VERSION, mapFunctionTargetRoleAffinities, matchCompanyRegistry, scoreCandidateDataQuality, scoreRecipientRelevance, SPECIFIC_ROLE_TAXONOMY, type SimulationCandidateSourceRecord, type NormalizedCandidateRecord } from "@/domain/candidates";
...
export function normalizeCandidateSources(records:readonly SimulationCandidateSourceRecord[],registry:readonly TargetCompany[],config:CandidatePipelineConfig=DEFAULT_CANDIDATE_PIPELINE_CONFIG):PlanningCandidate[]{
  ...
  for(const record of ordered){
    ...
    const classification=classifyCandidate(input),recipientFunction=classifyRecipientFunction(input.professionalTitle,input.industry),targetRoleAffinities=mapFunctionTargetRoleAffinities(recipientFunction),recipientRelevance=scoreRecipientRelevance(recipientFunction,classification.desiredRoleId??null),strategicRoleId=classification.desiredRoleId??targetRoleAffinities[0]?.targetRoleId,strategicRoleFamily=strategicRoleId?SPECIFIC_ROLE_TAXONOMY[strategicRoleId as keyof typeof SPECIFIC_ROLE_TAXONOMY]:undefined;
    ...
    const prohibitedReviewCode=(["c-suite-rejected","entry-level-peer-rejected","vp-not-selective-fit"] as const).find((code)=>classification.reviewCode===code);
    const functionAccepted=recipientFunction.reviewState==="accepted"&&recipientRelevance.score>=40,nonRoleAmbiguity=!classification.personaId?"persona-ambiguous":!classification.industryId?"industry-ambiguous":!classification.geographyId?"geography-ambiguous":null;
    if(!rejectionCode&&prohibitedReviewCode)rejectionCode=prohibitedReviewCode;
    if(!rejectionCode&&derived.some((value,index)=>value!==claimed[index]))rejectionCode="classification-conflict";
    if(!rejectionCode&&nonRoleAmbiguity)rejectionCode=nonRoleAmbiguity;
    ...
    const target=!rejectionCode&&company&&role&&persona&&industry?{id:input.internalId,company,role,preciseRoleId:classification.desiredRoleId??null,recipientRelevance:{score:recipientRelevance.score,version:recipientRelevance.version},persona,industry,geographyScore:geography?.score??0,roleAlignment:normalized.roleAlignment,functionalRelevance:normalized.functionalRelevance,yearsExperience:normalized.yearsExperience??0,sharedSignal:normalized.sharedSignal,dataQuality:normalized.dataQualityScore,roleSpecificUpside:normalized.roleSpecificUpside}:null;
