...
import {CONTROLLED_REAL_BATCH_COMPANIES,CONTROLLED_REAL_BATCH_MAX_ENRICHMENTS,ControlledRealBatchTransport,runControlledRealCandidateBatch,selectControlledEnrichmentShortlist} from "./run-controlled-real-batch";
...
describe("controlled real candidate batch",()=>{
  it("diversifies before enrichment and enforces immutable HTTP endpoint caps",async()=>{
    const snapshots=CONTROLLED_REAL_BATCH_COMPANIES.flatMap((company,index)=>Array.from({length:4},(_,person)=>({source:source(`${company.id}-${person}`,company.domain,index+person,"search"),recipientFunction:{reviewState:"accepted"},recipientRelevance:{score:80-person},personaId:"senior-ic",geographyId:"boston-ma",strategyCompanyMatch:{companyId:company.id},gateFailures:[]}))) as never[];
  ...
  it("caps search/enrichment, preserves exact IDs, and computes the observed credit delta",async()=>{
    ...
    expect(preview.plan.selected.length).toBeGreaterThan(0);expect(preview.plan.selected.length).toBeLessThanOrEqual(8);expect(preview.plan.target-preview.plan.selected.length).toBeGreaterThan(0);expect(preview.drafts).toHaveLength(preview.plan.selected.length);expect(preview.drafts.every((item)=>item.draft.status==="draft-only-simulation")).toBe(true);
