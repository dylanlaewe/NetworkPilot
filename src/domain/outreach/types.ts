...
export const OUTREACH_EVENT_TYPES = ["qualified", "rejected", "selected", "drafted", "simulated-sent", "actually-sent", "operator-confirmed-manual-send", "gmail-send-confirmed", "operator-reported-hard-bounce", "replied", "suppressed", "cancelled"] as const;
export type OutreachEventType = (typeof OUTREACH_EVENT_TYPES)[number];
export const CONTACT_IMPACTING_EVENT_TYPES: ReadonlySet<OutreachEventType> = new Set(["simulated-sent", "actually-sent", "operator-confirmed-manual-send", "gmail-send-confirmed"]);
export const HARD_BOUNCE_EVENT_TYPE = "operator-reported-hard-bounce" as const;
