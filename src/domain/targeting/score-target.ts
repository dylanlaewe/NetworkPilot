...
export const TARGETING_FUNCTION_SCORE_VERSION = "targeting-v2";
...
export function scoreTarget(candidate: TargetCandidate, weights: TargetingWeights=DEFAULT_TARGETING_WEIGHTS): TargetingScore {
  ...
  const version=candidate.recipientRelevance?TARGETING_FUNCTION_SCORE_VERSION:TARGETING_SCORE_VERSION,functionalRelevance=candidate.recipientRelevance?.score??candidate.functionalRelevance,roleAlignment=candidate.preciseRoleId?candidate.roleAlignment:candidate.recipientRelevance?50:candidate.roleAlignment;
  if (roleAlignment<40 || functionalRelevance<40) return { eligible:false,total:0,version,components:[],explanationCodes:["recipient-function-unrelated"],rejectionCode:"recipient-function-unrelated" };
  if (candidate.company.tier==="tier-3" && candidate.roleSpecificUpside<85) return { eligible:false,total:0,version,components:[],explanationCodes:["tier-3-needs-exceptional-fit"],rejectionCode:"tier-3-needs-exceptional-fit" };
  ...
  const definitions: Array<[ScoreComponent["id"],number,number,string]> = [
    ["company-desirability",companyValue,weights.companyDesirability,"company-reviewed-and-desirable"],["desired-role-alignment",roleAlignment,weights.desiredRoleAlignment,candidate.preciseRoleId?"precise-target-role-fit":"function-affinity-without-role-claim"],["recipient-functional-relevance",functionalRelevance,weights.recipientFunctionalRelevance,`recipient-function-relevant:${candidate.recipientRelevance?.version??"legacy"}`],["recipient-experience",experience,weights.recipientExperience,"recipient-has-experience-advantage"],["industry-priority",candidate.industry.score,weights.industryPriority,candidate.industry.tier==="primary"?"primary-industry":"secondary-industry"],["geographic-relevance",candidate.geographyScore,weights.geographicRelevance,"geography-preference"],["shared-signal",candidate.sharedSignal,weights.sharedSignal,"shared-career-signal"],["data-quality",candidate.dataQuality,weights.dataQuality,"data-complete"],
  ];
  ...
  return { eligible:true,total:Number(components.reduce((sum,c)=>sum+c.weighted,0).toFixed(2)),version,components,explanationCodes:components.map((c)=>c.explanationCode) };
}
