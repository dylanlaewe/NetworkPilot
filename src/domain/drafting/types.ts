...
export type OutreachLane="data-analytics"|"engineering-technical"|"product-management"|"project-operations"|"consulting"|"finance"|"commodities-energy"|"defense-technology"|"career-path-leader";
export interface DraftTemplate { outreachIntent?:IntentSelection; id:string; laneId:OutreachLane; variantId:"direct-practical"|"career-curiosity"|"common-ground"; catalogVersion:string; version:string; displayName:string; fragmentIds:string[]; factIds:string[]; structure:"facts-first"|"reason-first"|"experience-first"; subject:string; reason:string; question:string; }
export interface DraftRecipient { intentEvidence?:{verified:boolean;relationship:"hands-on-data-systems";sourceReference:string}; id:string; firstName:string; companyName:string; roleFamilyId:string; industryId:string; personaId:string; professionalTitle?:string; primaryRecipientFunction?:string; }
export interface DraftContext { runId:string; }
export interface DraftTemplateSelection {template:DraftTemplate;reason:string;}
export interface RenderedDraft { outreachIntent:IntentSelection; subject:string; body:string; templateId:string; templateVersion:string; templateCatalogVersion:string; referencedFactIds:string[]; evidenceIds:string[]; generatedAt:string; status:"draft-only-simulation"; usedEvidence:boolean; }
